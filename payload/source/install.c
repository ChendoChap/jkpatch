...
int install_payload(struct thread *td, uint64_t kernbase, void *payload, size_t psize) {
	...
	*(uint8_t *)(kernbase + 0x16DFEC) = 7; // VM_PROT_ALL;
	*(uint8_t *)(kernbase + 0x16E002) = 7; // VM_PROT_ALL;
	__writecr0(CR0);
	...
	__writecr0(CR0 & ~CR0_WP);
	*(uint8_t *)(kernbase + 0x16DFEC) = 3; // VM_PROT_DEFAULT;
	*(uint8_t *)(kernbase + 0x16E002) = 3; // VM_PROT_DEFAULT;
	__writecr0(CR0);
